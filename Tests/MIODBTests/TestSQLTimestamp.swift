...
final class TestSQLTimestamp: XCTestCase
{
    ...
    func testRoundTripTextToText () {
        ...
        for s in samples {
            XCTAssertEqual( MDBSQLTimestampString( parse( s )! ), s, "round trip of \(s)" )
        }
    }

    /// Date -> text -> Date is identity to microsecond precision.
    func testRoundTripDateToDate () {
        ...
        for t in samples {
            ...
            XCTAssertEqual( back.timeIntervalSince1970, d.timeIntervalSince1970, accuracy: 1e-6, "round trip of \(t)" )
        }
    }
}
